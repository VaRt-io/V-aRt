...
class VRNightScene extends Component{
    ...
    const pedRotations = ["90 -90 0", "90 -145 0", "90 -215 0", "90 -270 0", "90 -335 0", "90 -35 0"];
    ...
    function getPaintingName(pName) {
    ...
    return (
      paintings?
        <a-scene>
            ...
            <a-sphere src={cyberRust} position="-22.232 11.695 35.686" radius="1.25" />

            <a-torus-knot src={cyberRust} position ="-18.379 1.936 -17.890"  />
            <a-dodecahedron src={circutBoard} position= "-1 13 -32"  />

            { paintings && paintings.map((image, index) => {
                        return(
                          <a-text key={image.id} value={getPaintingName(image.name)} align="center" position={textPositions[index]} rotation={textRotations[index]} color="white" />
            )})
            }

            { paintings && paintings.map((image, index) => {
                        return(
                          <a-entity key={image.id} obj-model='obj:#column-obj; mtl:#column-mtl' position={pedPositions[index]} scale=".001 .001 .001" rotation={pedRotations[index]} scale="5 5 5"/>
            )})
            }

            { paintings && paintings.map((image, index) => {
                        return(
                          <a-entity key={image.id} geometry={{primative: 'box'}} material={`src: ${paintings[index].url}`} position={boxPositions[index]} rotation="0 90 0" href>
